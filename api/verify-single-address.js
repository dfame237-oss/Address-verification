...
/**
 * Core logic for running verification, post-processing, and generating remarks.
 * This function is now REUSABLE for both single and bulk processing.
 */
async function runVerificationLogic(address, customerName) {
    ...
    if (geminiResult.error || !geminiResult.text) {
        ...
        return {
            ...
            addressLine1: address, landmark: "", state: "", district: "", pin: initialPin, success: false
        };
    }
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Map the result keys to match the old handler's final output (optional, but clean)
        const finalOutput = {
            status: finalResponse.status,
            customerRawName: finalResponse.customerRawName,
            customerCleanName: finalResponse.customerCleanName,
            addressLine1: finalResponse.addressLine1,
            landmark: finalResponse.landmark,
            postOffice: finalResponse.postOffice,
            tehsil: finalResponse.tehsil,
            district: finalResponse.district,
            state: finalResponse.state,
            pin: finalResponse.pin,
            addressQuality: finalResponse.addressQuality,
            locationType: finalResponse.locationType,
            locationSuitability: finalResponse.locationSuitability,
            remarks: finalResponse.remarks,
        };
...
// Export core logic functions for use by the bulk job processor
...
module.exports.directionalKeywords = directionalKeywords; // Export for completeness