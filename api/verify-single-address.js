...
module.exports = async (req, res) => {
    ...
    // POST: process verification with credits logic (Bulk Verification no longer uses this, but single verification does)
    if (req.method === 'POST') {
        // Authenticate
        const jwtPayload = parseJwtFromHeader(req);
        if (!jwtPayload || !jwtPayload.clientId) {
        ...
        try {
            ...
            let reserved = false;
            if (!isUnlimited) {
                // Atomically decrement remainingCredits if it's > 0 (DEDUCTION FOR SINGLE CALL)
            ...
            const finalResponse = {
                ...
                addressLine1: parsedData.FormattedAddress ||
                address.replace(meaninglessRegex, '').trim() || '', 
