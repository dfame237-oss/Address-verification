...
//
  ...
  const bulkJobs = db.collection('bulkJobs');
  const creditDeductions = db.collection('creditDeductions');
// NEW: Permanent deduction log
...
return sendJSON(res, 500, { status: 'Error', message: 'Internal server error fetching job count.' });
    ...
    // -------------------------
    // ACTION: deduction-history (GET) - PAGINATED, MAX 90 DAYS (CRITICAL CHANGE)
    // -------------------------
    if (action === 'deduction-history') {
        if (req.method !== 'GET') return sendJSON(res, 405, { status: 'Error', message: 'Method Not Allowed' });
        
        // --- NEW PAGINATION & DATE LOGIC ---
        const page = parseInt(url.searchParams.get('page')) || 1; // Default to page 1
        const limit = 15; // 15 entries per page
        const skip = (page - 1) * limit;
        const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000); // Max 90 days of history
        ...
        try {
            ...
            const totalCount = await creditDeductions.countDocuments(filter);
            const totalPages = Math.ceil(totalCount / limit);
            
            // 2. Fetch the paginated history data
            const deductionHistory = await creditDeductions.find(filter)
                .sort({ timestamp: -1 }) // Sort by newest deduction first
                .skip(skip) // Apply skip for pagination
                .limit(limit) // Apply limit for pagination
                .toArray();

            // Return history along with pagination metadata
            return sendJSON(res, 200, { 
                status: 'Success', 
