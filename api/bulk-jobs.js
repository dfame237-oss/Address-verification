...
= await runVerificationLogic(rawAddress, customerName); 
    ...
    // --------------------------------------------------------
    // *** MODIFIED FILE CLASSIFICATION LOGIC ***
    // --------------------------------------------------------
    ...
    // New array to identify remarks that should be sanitized for the user
    ...
    for (const row of allProcessedRows) {
        ...
        // Includes: Fatal Error (status: Error), Missing Address (status: Skipped), or Contains Email (status: Skipped).
        // Testing Order (status: Success, remarks: "Testing Order") is also considered a skip for shipping.
        if (row.status !== "Success" || row.remarks.includes('Testing Order')) {
            manualCheckRows.push(row);
            continue;
        }

        // --- Classification Pre-Checks ---
        // Condition 1: Address Quality is Good or Very Good or Medium
        const isHighQuality = ['Very Good', 'Good', 'Medium'].includes(row.addressQuality);
        
        // Condition 2: Check for any CRITICAL keyword in the remarks (e.g., Wrong PIN, PIN not found, P.O. conflict)
        const hasCriticalKeyword = criticalKeywords.some(keyword => row.remarks.includes(keyword));
        
        // --- Classification Rule 2: Ready for Ship ---
        // Condition: (Quality is Good/Very Good/Medium) AND (NO Critical Remarks)
        if (isHighQuality && !hasCriticalKeyword) {
            readyForShipRows.push(row);
        } else {
            // --- Manual Check Rule 3: Low Quality OR Critical Remark Present ---
            
            let sanitizedRow = { ...row }; // Copy the row for potential remark replacement
            let remarkNeedsSanitizing = false;

            // Check if any AI-related remark is present
            for (const aiRemark of AI_RELATED_REMARKS) {
                if (row.remarks.includes(aiRemark)) {
                    remarkNeedsSanitizing = true;
                    // Do not break here, we want to replace all instances of AI-related remarks
                }
            }
            
            // If an AI-related error is found, replace the specific AI references with a generic message
            if (remarkNeedsSanitizing) {
                // Ensure the remark doesn't reveal the specific AI issue
                let newRemarks = row.remarks;
                // Use a general replacement pattern that doesn't rely on specific keyword matching in the loop
                ...
                // Final cleanup of multiple generic errors (e.g., if multiple AI issues occurred)
...
// EXPORTS
module.exports.runJobProcessor = runJobProcessor;