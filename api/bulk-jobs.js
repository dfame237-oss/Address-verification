...
                const result 
= await runVerificationLogic(rawAddress, customerName); 
                
                if (result.success) {
...
    // --------------------------------------------------------
    // *** MODIFIED FILE CLASSIFICATION LOGIC (STRICT) ***
    // --------------------------------------------------------

    const readyForShipRows = [];
    const manualCheckRows = [];
    
    // CRITICAL_KEYWORDS is imported from verify-single-address.js
    const criticalKeywords = CRITICAL_KEYWORDS;

    // List of internal AI errors that need sanitization (mapped to GENERIC_AI_ERROR_REMARK)
    const AI_RELATED_REMARKS = [
        'JSON parse failed', 
        'Address could not be verified by Gemini', 
        'Network issue contacting AI service', 
        'External AI verification service failure', 
        'No candidates found',
        'Name cleaning failed'
    ];
    // Generic message to hide internal AI/Gemini errors
    const GENERIC_AI_ERROR_REMARK = 'Internal Verification Warning: Data Conflict.';

    for (const row of allProcessedRows) {
        
        // --- Manual Check Rule 1: Immediate Failure/Skip ---
        // Includes: Fatal Error (status: Error), Missing Address (status: Skipped), Invalid Email (status: Skipped).
        if (row.status !== "Success") {
            manualCheckRows.push(row);
            continue;
        }

        // --- NEW STRICT RULE: Check for ANY warning phrase or correction ---
        // We check for: 
        // 1. Explicitly negative status/remarks (Wrong PIN, Testing Order).
        // 2. The explicit word 'Warning' (from Short Address or PIN not found).
        // 3. Any of the CRITICAL_ALERT prefixes (if the single-address script used them).
        
        const remarksLower = row.remarks.toLowerCase();
        
        const hasAnyWarning = remarksLower.includes('wrong pin') ||
                              remarksLower.includes('warning') ||
                              remarksLower.includes('testing order') ||
                              criticalKeywords.some(keyword => remarksLower.includes(keyword.toLowerCase())) ||
                              AI_RELATED_REMARKS.some(keyword => remarksLower.includes(keyword.toLowerCase())); // Catch unsanitized AI errors

        // --- Classification Rule 2: Ready for Ship ---
        // CONDITION: Must be status: Success AND have NO WARNINGS/CRITICAL REMARKS
        if (row.status === 'Success' && !hasAnyWarning) {
            readyForShipRows.push(row);
        } else {
            // --- Manual Check Rule 3: Process the row for Manual Check (with sanitization) ---
            
            let sanitizedRow = { ...row }; // Copy the row for potential remark replacement
            let remarkNeedsSanitizing = false;

            // Check if any AI-related remark is present (for sanitization purposes only)
            for (const aiRemark of AI_RELATED_REMARKS) {
                if (remarksLower.includes(aiRemark.toLowerCase())) {
                    remarkNeedsSanitizing = true;
                }
            }
            
            // If an AI-related error is found, replace the specific AI references with a generic message
            if (remarkNeedsSanitizing) {
                let newRemarks = row.remarks;
                
                AI_RELATED_REMARKS.forEach(keyword => {
                    // Replace the keyword with the generic error message
                    newRemarks = newRemarks.replace(new RegExp(keyword, 'gi'), GENERIC_AI_ERROR_REMARK);
                });
                
                // Cleanup duplicate separators
                newRemarks = newRemarks.replace(new RegExp(`(${GENERIC_AI_ERROR_REMARK};\\s*)+`, 'g'), `${GENERIC_AI_ERROR_REMARK}; `).trim();
                if (newRemarks.endsWith(';')) newRemarks = newRemarks.slice(0, -1);
                
                sanitizedRow.remarks = newRemarks.trim();
            }
            
            manualCheckRows.push(sanitizedRow);
        }
    }
